...
export async function getPostByName(
  ...
  const res = await fetch(
    `https://raw.githubusercontent.com/Vishal-Chandrasekaran/test-mdx-blogposts/main/${fileName}`,
    {
      headers: {
...
export async function getPostsMeta(): Promise<Meta[] | undefined> {
  ...
  const res = await fetch(
    "https://api.github.com/repos/Vishal-Chandrasekaran/test-mdx-blogposts/git/trees/main?recursive=1",
    {
      headers: {
        Accept: "application/vnd.github+json",
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
